...
// УНИВЕРСАЛЬНЫЙ СОВМЕСТИМЫЙ РИСОВАЛЬЩИК КНОПОК/ПЛАШЕК ДЛЯ СТАРЫХ УСТРОЙСТВ
...
// Получаем массив игроков с секторами на круге 360 градусов (в радианах)
...
function animateDeterministicBall() {
    ...
    if (elapsed < 3000) {
        ...
        // Стрелка указывает точно по траектории будущего запуска
    ...
    if (frameIndex < currentPath.length) {
        ...
        // Полная остановка: подсвечиваем выигравший сектор
...
function renderWheelSections(drawArrow = false, arrowAngle = 0, highlightWinner = false) {
    ...
    segments.forEach((p, index) => {
        ctx.beginPath();
        ctx.moveTo(200, 200);
        ctx.arc(200, 200, 300, p.startAngle, p.endAngle);
        ctx.closePath();

        ctx.fillStyle = p.color;
        ctx.fill();

        // Если сектор победил — включаем мерцающую обводку (контур) и пульсирующую заливку
        if (highlightWinner && index === winningSegIndex) {
            const isFlashOn = Math.floor(Date.now() / 200) % 2 === 0;
            
            // Вспышка заливки сектора
            ctx.fillStyle = isFlashOn ? 'rgba(255, 255, 255, 0.4)' : 'rgba(255, 255, 255, 0.1)';
            ctx.fill();

            // Создаем жирную белую светящуюся неоновую обводку
            ctx.save();
            ctx.lineWidth = 10;
        ...
        // 2. Пишем только никнеймы игроков (БЕЗ аватарок и БЕЗ сумм ставок)
    ...
    // НИК НАД ЛЕТЯЩИМ ШАРИКОМ (С ПОДЛОЖКОЙ, ПОЛНОСТЬЮ СОВМЕСТИМОЙ С TELEGRAM)
...
// ======= ИГРА 6: ВЗЛЕТ РАКЕТЫ V3 (GPU ОПТИМИЗИРОВАНО) =======
